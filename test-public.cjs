...
assert.equal(d.stats.claude_content_read,1117);
...
assert.equal(d.sourceNodes.length,97);assert.equal(d.sourceLinks.length,177);
...
assert.equal(getView().ns.length,115,'總覽完整呈現三層');
assert.deepEqual([...new Set(d.nodes.map(n=>n.level))].sort(),[1,2,3]);
assert.equal(d.links.filter(l=>l.type==='分類').length,108);
...
assert.equal(reached.size,115,'完整主圖必須連通，不可仍是七座孤島');
...
console.log(`通過：115 節點三層全貌、單一連通網路、${bridgePairs.size} 條有原始依據且不重複的共現線索、53 舊專案保留、搜尋期間及隱私。`);
