...
assert.equal(d.start,'2026-06-01');assert.equal(d.until,'2026-09-10');
...
assert.equal(d.days.length+d.missing_dates.length,102);
assert.equal(d.stats.claude_content_pending,0,'本次要求的 Claude 文件內容仍有未讀');
assert.equal(d.stats.claude_content_read,1106);
...
assert.equal(d.sourceNodes.length,90);assert.equal(d.sourceLinks.length,160);
...
assert.equal(getView().ns.length,108,'總覽完整呈現三層');
assert.deepEqual([...new Set(d.nodes.map(n=>n.level))].sort(),[1,2,3]);
assert.equal(d.links.filter(l=>l.type==='分類').length,101);
...
assert.equal(reached.size,108,'完整主圖必須連通，不可仍是七座孤島');
...
console.log(`通過：108 節點三層全貌、單一連通網路、${bridgePairs.size} 條有原始依據且不重複的共現線索、53 舊專案保留、搜尋期間及隱私。`);
